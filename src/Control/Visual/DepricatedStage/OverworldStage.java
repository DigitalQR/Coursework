...
public class OverworldStage extends Stage{
	...
	public static float getLERPTSime(){
		return LERPTime;
	}
	...
	public void update(){
		...
		if(winner == -1){
			...
			for(int i = 0; i<Settings.User.size(); i++){
				...
				font.setRGBA(1-Camera.getRGBA()[0], 1-Camera.getRGBA()[1], 1-Camera.getRGBA()[2], 1);
		...
		//If there is a winner
			...
			for(int i = 0; i<Settings.User.size(); i++){
				if(i != winner-1){
					...
					Camera.r = Settings.User.get(i).getRGBA()[0];
					Camera.g = Settings.User.get(i).getRGBA()[1];
					Camera.b = Settings.User.get(i).getRGBA()[2];
				}
			}
